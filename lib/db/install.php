...
trait install
{
	...
	public static function install($_onlyUpgrade = false, $_addonsFirst = true)
	{
		...
		foreach ($myList as $myDbLoc => $myDbName)
		{
			...
			if($_onlyUpgrade)
			{
				...
				$result[$myDbName]['exec']    = self::execFolder($myDbLoc.'/', 'v.', false, $myDbCon);
			}
	 ...
	 */
	public static function execFile($_path, $_addons = false, $_db_name = true)
	{
	 ...
	 * @return [type]           [description]
	 */
	public static function execFolder($_path = null, $_group = null, $_addons = false, $_db_name = true)
	{
		$result = [];
		// if want to read from addons update location
		$myDbName = null;
		...
		{
			$result[$filename] = self::execFile($filename, false, $myDbName);
		}
